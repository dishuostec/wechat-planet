...
class Environment
{
    ...
    public function __construct($mode = NULL, $options = array())
    {
        ...
        $this->_config_dir = $this->getOption('config_dir',
            ROOT_DIR.'/protected/config');
        ...
        //非开发环境 从缓存中读取配置
        if (substr($this->_mode, 0, 3) != 'DEV'
            and isset($options['life_time'])
        ) {
            $this->_cache_dir = $this->getOption('cache_dir',
                ROOT_DIR.'/protected/runtime/cache');
    ...
    public function setDebug()
    {
        ...
        defined('MOD_TRACE_LEVEL')
        or define('MOD_TRACE_LEVEL', $this->getTraceLevel());
    }
    ...
    public function loadConfig()
    {
        switch ($this->_mode) {
            case "DEVELOPMENT":
                //            print_r($this->_development());
                $this->_config = self::mergeArray($this->_main(),
                    $this->_development());
                $this->_debug = TRUE;
                $this->_trace_level = 3;
                break;
        ...
        $this->_config['params']['environment'] = defined("self::".$this->_mode)
            ? constant("self::".$this->_mode) : 0;
    }
    ...
    {
        //        echo $this->_config['params']['modPath'];
