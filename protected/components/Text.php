...
class Text
{
     ...
     * @uses    UTF8::strlen
     */
    public static function limit_chars($str, $limit = 100, $end_char = NULL,
    ...
    public static function random($type = NULL, $length = 8)
    {
        ...
        $pool = ($utf8 === TRUE) ? UTF8::str_split($pool, 1)
            : str_split($pool, 1);
     ...
     * @uses    UTF8::strlen
     */
    public static function censor($str, $badwords, $replacement = '#',
                                  $replace_partial_words = TRUE)
    {
        foreach ((array) $badwords as $key => $badword) {
            $badwords[$key] = str_replace('\*', '\S*?',
                preg_quote((string) $badword));
        }
        ...
        if (UTF8::strlen($replacement) == 1) {
            ...
            return preg_replace($regex,
                'str_repeat($replacement, UTF8::strlen(\'$1\'))', $str);
        }
    ...
    public static function auto_link_urls($text)
    {
        ...
        $text = preg_replace_callback('~\b(?<!href="|">)(?:ht|f)tps?://[^<\s]+(?:/|\b)~i',
            'Text::_auto_link_urls_callback1', $text);

        // Find and replace all naked www.links.com (without http://)
        return preg_replace_callback('~\b(?<!://|">)www(?:\.[a-z0-9][-a-z0-9]*+)+\.[a-z]{2,6}[^<\s]*\b~i',
            'Text::_auto_link_urls_callback2', $text);
    }
    ...
    public static function auto_link_emails($text)
    {
        ...
        return preg_replace_callback('~\b(?<!href="mailto:|58;)(?!\.)[-+_a-z0-9.]++(?<!\.)@(?![-.])[-a-z0-9.]+(?<!\.)\.[a-z]{2,6}\b(?!</a>)~i',
     ...
     * @return  string
     */
    public static function bytes($bytes, $force_unit = NULL, $format = NULL,
                                 $si = TRUE)
    {
        // Format string
        ...
        if ($value === 'browser' OR $value == 'version') {
            ...
            foreach ($browsers as $search => $name) {
                if (stripos($agent, $search) !== FALSE) {
                    ...
                    if (preg_match('#'.preg_quote($search)
                    .'[^0-9.]*+([0-9.][0-9.a-z]*)#i', Request::$user_agent,
                        $matches)
                    ) {
