...
class CFileCache extends CCache
{
    ...
    public function init()
    {
        ...
        if ($this->cachePath === NULL) {
            $this->cachePath =
                Mod::app()->getRuntimePath().DIRECTORY_SEPARATOR.'cache';
        }
    ...
    protected function setValue($key, $value, $expire)
    {
        ...
        if (@file_put_contents($cacheFile,
                $this->embedExpiry ? $expire.$value : $value, LOCK_EX) !== FALSE
        ) {
    ...
    public function gc($expiredOnly = TRUE, $path = NULL)
    {
        ...
        while (($file = readdir($handle)) !== FALSE) {
            ...
            } elseif ($expiredOnly && $this->filemtime($fullPath) < time()
                || ! $expiredOnly
            ) {
                @unlink($fullPath);
            }
